...
> 诱因：我们需要在浏览器的环境下面去验证我们的代码渲染逻辑是否正确
> 方法: 使用rollup打包工具进行整个项目的打包

- 下载rollup 并创建rollup.config.js

NOTE: 引入 json 文件的时候 需要使用 assert 来断言 json文件的格式，不然打包会报错
...
```json
//tsconfig.json 通过 npx tsc --init 生成的
{
  "compilerOptions": {
